...
/**
 * Enhanced Lavalink client using Riffy
...
module.exports = (client) => {
  ...
  const riffy = new Riffy(client, nodes, {
    ...
    defaultSearchPlatform: client.config.MUSIC.DEFAULT_SOURCE === 'YT' ? 'ytsearch' : 
                          ...
                          'scsearch',
    restVersion: 'v4', // Using Lavalink v4
  ...
  client.once("ready", () => {
    console.log(`📡 Initializing Riffy Lavalink client with User ID: ${client.user.id}`);
  ...
  riffy.on("nodeConnect", (node) => {
    client.logger.log(`${emojiManager.getSuccess()} Riffy node "${node.name}" connected`);
  });

  riffy.on("nodeDisconnect", (node, reason) => {
    client.logger.warn(`${emojiManager.warning} Riffy node "${node.name}" disconnected: ${reason}`);
  });

  riffy.on("nodeError", (node, error) => {
    client.logger.error(`${emojiManager.getError()} Riffy node "${node.name}" error: ${error.message}`);
  });

  riffy.on("nodeReconnect", (node) => {
    client.logger.log(`🔄 Riffy node "${node.name}" reconnecting...`);
  });
  ...
  riffy.on("trackStart", async (player, track) => {
    ...
    try {
      // Generate beautiful visual card with timeout for speed (7 seconds)
  ...
  // Add compatibility methods for backward compatibility with lavaclient API
  ...
  riffy.createPlayer = (guildId) => {
    return riffy.createConnection({
      guildId: guildId,
      voiceChannel: null, // Will be set when connecting
      textChannel: null,  // Will be set when connecting
      deaf: true
    });
  };
  ...
  // Spotify integration (Riffy has built-in support with proper Lavalink setup)
  if (process.env.SPOTIFY_CLIENT_ID && process.env.SPOTIFY_CLIENT_SECRET) {
    console.log(`${emojiManager.getSuccess()} Spotify credentials found - Riffy will handle Spotify tracks via Lavalink`);
  }
